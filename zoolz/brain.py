...
def generate_zoolz_reply(message: str, status_fetcher=None) -> Dict[str, str]:
    """
    Generate a lightweight, offline-friendly reply.
    ...
    if not text:
        return {"reply": "Hit me with anything about ZoolZ, modeling, or server status."}
    ...
    if 'ai' in lower or 'brain' in lower or 'chat' in lower:
        reply_parts.append("This chat is local-only and rule-based. A small model can be wired later without external APIs.")
