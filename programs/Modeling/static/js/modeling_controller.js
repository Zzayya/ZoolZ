...
async function generateCookieCutter() {
    ...
    try {
        // Try async endpoint first (if Celery is available)
        const response = await fetch('/modeling/api/generate_async', {
            method: 'POST',
            body: formData
        });
        ...
        if (response.status === 503) {
            ...
            console.log('⚠️ Celery not available, using synchronous generation');
            return generateCookieCutterSync(formData, generateBtn, generateText, downloadBtn);
        }

        if (data.success && data.task_id) {
            // Background task started - poll for progress
            showStatus('Generation started in background...', 'processing');
            pollTaskProgress(data.task_id, generateBtn, generateText, downloadBtn);
        } else {
...
function openInCura() {
    if (!downloadUrl) {
        showNotification('No model to open', 'error');
        return;
    }

    // Get the full URL to the STL file
    const fullUrl = window.location.origin + downloadUrl;

    // Try to open Cura with cura:// protocol handler
    // This requires Cura to be installed and configured
    const curaUrl = `cura://open?file=${encodeURIComponent(fullUrl)}`;

    showNotification('Opening in Cura...', 'info');

    // Try to open with cura:// protocol
    const link = document.createElement('a');
    link.href = curaUrl;
    link.click();

    // Fallback: provide instructions if protocol doesn't work
    setTimeout(() => {
        showNotification('If Cura didn\'t open, download the file and open manually', 'info');
    }, 2000);
}
...
window.addEventListener('load', () => {
    ...
    // Setup widen hole control toggles
    const autoDetectCheckbox = document.getElementById('widenHoleAutoDetect');
    const partialCheckbox = document.getElementById('widenHolePartial');

    if (autoDetectCheckbox) {
        autoDetectCheckbox.addEventListener('change', function() {
            const manualControls = document.getElementById('widenholeManualControls');
            if (manualControls) {
                manualControls.style.display = this.checked ? 'none' : 'block';
            }
        });
    }
    ...
    if (partialCheckbox) {
        partialCheckbox.addEventListener('change', function() {
            const partialControls = document.getElementById('widenholePartialControls');
            if (partialControls) {
                partialControls.style.display = this.checked ? 'block' : 'none';
            }
        });
    }
});
...
// WIDEN HOLE TOOL
// ============================================================================
async function applyWidenHole() {
    if (!currentFile) {
        showNotification('Please load an STL file first', 'error');
        return;
    }

    const newRadius = parseFloat(document.getElementById('widenHoleRadius').value);
    const autoDetect = document.getElementById('widenHoleAutoDetect').checked;
    const partial = document.getElementById('widenHolePartial').checked;

    const formData = new FormData();
    formData.append('stl', currentFile);
    formData.append('new_radius', newRadius);
    formData.append('auto_detect', autoDetect);

    // Manual mode parameters
    if (!autoDetect) {
        const currentRadius = parseFloat(document.getElementById('widenHoleCurrentRadius').value);
        const axis = parseInt(document.getElementById('widenHoleAxis').value);
        formData.append('current_radius', currentRadius);
        formData.append('axis', axis);
    }

    // Partial widening parameters
    if (partial) {
        const heightMin = parseFloat(document.getElementById('widenHoleHeightMin').value);
        const heightMax = parseFloat(document.getElementById('widenHoleHeightMax').value);
        formData.append('height_min', heightMin);
        formData.append('height_max', heightMax);
    }

    showNotification('Widening hole...', 'info');

    try {
        const response = await fetch('/modeling/api/stl/widen_hole', {
            method: 'POST',
            body: formData
        });
        ...
        if (data.success) {
            ...
            document.getElementById('openInCuraBtn').style.display = 'block';

            pushUndoState('Widen Hole');

            showNotification(`✅ Hole widened to ${newRadius}mm!`, 'success');

            // Show stats
            if (data.stats) {
                console.log('Widen Hole Stats:', data.stats);
            }
        } else {
            showNotification('Error: ' + data.error, 'error');
        }
    } catch (error) {
        showNotification('Error widening hole: ' + error.message, 'error');
        console.error(error);
    }
}
